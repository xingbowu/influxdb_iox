...
mod tests {
    ...
    async fn test_size_estimation() {
        ...
        let single_file_size = 1102;
        let two_file_size = 2171;
    ...
    async fn test_max_persisted_sequence_number() {
        ...
        cache.expire_if_unknown(table_id, Some(sequence_number_2));
        assert_eq!(
            cache.get(table_id).await.ids(),
            ids(&[&tfile1_2, &tfile1_3])
        );
        ...
        // should not expire anything
        cache.expire_if_unknown(table_id, None);
        assert_eq!(
            cache.get(table_id).await.ids(),
            ids(&[&tfile1_2, &tfile1_3])
        );
        ...
        cache.expire_if_unknown(table_id, Some(sequence_number_10));
