...
mod tests {
    ...
    async fn test_parquet_chunks() {
        ...
        let file112 = partition11
            .create_parquet_file_with_min_max("table1 foo=2 22", 3, 4, now, now)
            .await;
        let file113 = partition11
            .create_parquet_file_with_min_max("table1 foo=3 33", 5, 6, now, now)
            .await;
        let file114 = partition11
            .create_parquet_file_with_min_max("table1 foo=4 44", 7, 8, now, now)
            .await;
        let file115 = partition11
            .create_parquet_file_with_min_max("table1 foo=5 55", 9, 10, now, now)
            .await;
        let file121 = partition12
            .create_parquet_file_with_min_max("table1 foo=5 55", 1, 2, now, now)
            .await;
        let _file211 = partition21
            .create_parquet_file_with_min_max("table2 foo=6 66", 1, 2, now, now)
            .await;
    ...
    async fn test_compactor_collision() {
        ...
        partition
            .create_parquet_file_with_min_max("table foo=1 11", 1, 2, now, now)
            .await;
        ...
        let err = chunks_with_ingester_partition(&querier_table, ingester_partition)
            .await
            .unwrap_err();
    ...
    async fn test_state_reconcile() {
        ...
        partition1
            .create_parquet_file_with_min_max("table foo=2 22", 3, 3, now, now)
            .await;

        // kept because max sequence number <= 3
        let file2 = partition2
            .create_parquet_file_with_min_max("table foo=1 11", 1, 3, now, now)
            .await;

        // pruned because min sequence number > 3
        partition2
            .create_parquet_file_with_min_max("table foo=2 22", 4, 4, now, now)
            .await;
        ...
        let mut chunks = chunks_with_ingester_partitions(
            &querier_table,
            vec![
                // this chunk is kept
                builder1
    ...
    async fn test_ingester_overlap_detection() {
        ...
        let err = chunks_with_ingester_partitions(
            &querier_table,
            vec![
                builder1
                    .clone()
    ...
    async fn test_parquet_cache_refresh() {
        ...
        let chunks = chunks_with_ingester_partition(&querier_table, ingester_partition.clone())
            .await
            .unwrap();
        ...
        let chunks = chunks_with_ingester_partition(&querier_table, ingester_partition)
            .await
            .unwrap();
        assert_eq!(chunks.len(), 2);
    ...
    async fn test_tombstone_cache_refresh() {
        ...
        let deletes = num_deletes(
            chunks_with_ingester_partition(&querier_table, ingester_partition.clone())
                .await
                .unwrap(),
        );
        ...
        let deletes = num_deletes(
            chunks_with_ingester_partition(&querier_table, ingester_partition)
                .await
                .unwrap(),
        );
        assert_eq!(&deletes, &[1, 0]);
    ...
    async fn chunks_with_ingester_partition(
        ...
        chunks_with_ingester_partitions(querier_table, vec![ingester_partition]).await
    }

    /// Invokes querier_table.chunks modeling the ingester sending multiple partitions via `next_response`
    async fn chunks_with_ingester_partitions(
        querier_table: &QuerierTable,
        ...
        querier_table
            ...
            .unwrap()
            .next_response(Ok(ingester_partitions));

        querier_table.chunks(&pred).await
    }
